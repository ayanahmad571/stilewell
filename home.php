...
if ($getmainres->num_rows > 0) {
	...
	while($getmainrw = $getmainres->fetch_assoc()) {
		...
		echo '<tr>';
		  echo '<td>'.$con.'</td>
				<td><img data-original="'.$getmainrw['pr_img_2'].'" class="img-responsive lazy" /></td>
				<td>'.$getmainrw['pr_code'].' '.$getmainrw['pr_name'].'</td>
				<td>'.convert_desc($getmainrw['pr_desc']).'</td>
				<td>'.$getmainrw['prty_name'].'</td>
...
if (strlen(strstr($agent, 'Firefox')) > 0) {
	...
	?>
    <script type="text/javascript">
	alert("StileWell Software works best on Google Chrome browser. There might be some issues in printing Quotations in Firefox. Consider Switching to Chrome.");
	</script>
    <?php
}
?>
