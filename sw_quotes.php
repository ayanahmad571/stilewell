                                    ...
                                    <table id="datatable1" class="table table-striped table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width:20%">Client Billing </th>
                                                    <th style="width:10%">Client Payment Terms</th>
...
if(($_SESSION['STWL_LUM_DB_ID'] == 1) or ($_SESSION['STWL_LUM_DB_ID']  == 2)  ){
...
";
}elseif(($_SESSION['STWL_LUM_DB_ID'] == 10 )){
	$productsql = "SELECT * FROM `sw_quotes` q
left join sw_clients c on q.qo_rel_cli_id = c.cli_id
left join sw_currency y on q.qo_rel_cur_id = y.cur_id
WHERE q.qo_revision = 0 and q.qo_valid =1 and c.cli_valid =1 and
q.qo_rel_lum_id != 1 and q.qo_rel_lum_id  != 7
order by qo_ref desc
...
left join sw_currency y on q.qo_rel_cur_id = y.cur_id
WHERE q.qo_revision = 0 and q.qo_valid =1 and c.cli_valid =1 and
q.qo_rel_lum_id =".$_SESSION['STWL_LUM_DB_ID']."
order by qo_ref desc
...
if ($productres->num_rows > 0) {
	...
	*/
	echo '<td>';
	$getrevisions = "SELECT * FROM sw_quotes where qo_revision_id = ".$productrw['qo_id']." and qo_valid =1 order by qo_id asc";
