...
<link href="assets/datatables/jquery.dataTables.min.css" rel="stylesheet" type="text/css" />  
    ...
    <body>
        ...
        <section class="content">
            ...
            <!-- Header Ends -->
<?php
$checklastupdate = getdatafromsql($conn, "select * from sw_totals_max where tt_type = 'sales' and tt_dnt = '".date('d-m-y',time())."'");

if(is_array($checklastupdate)){
	
	$totalvaluetillnow = $checklastupdate['tt_val'];
}else{
...
$getallmaxsis = "SELECT * from sw_salesinvoices_max";
$getallmaxsis = $conn->query($getallmaxsis);

if ($getallmaxsis->num_rows > 0) {
    // output data of each row
    while($getallmaxsisrw = $getallmaxsis->fetch_assoc()) {
	$checkgen = getdatafromsql($conn,"select * from sw_salesinvoices_gen where sog_rel_so_id = ".$getallmaxsisrw['so_revision_id']." and sog_valid =1 order by sog_id desc limit 1 ");				
...
where si_rel_so_id = ".$getallmaxsisrw['so_id']." and si_valid =1 and prty_pr_hidden = 0");
if(is_array($checkgen)){
	$discount = $checkgen['sog_discount'];
	$vat =  ($checkgen['sog_vat']/100)* $getpricetotal['ptotal'];
	$extraprice = $checkgen['sog_extra_price'];
		 
	 }
	 $totalvaluetillnow += $getpricetotal['ptotal'] - $discount + $vat + $extraprice;
...
/*-------------------------------------------------------------------------------*/
$inserttime = "INSERT INTO `sw_totals_max`(`tt_type`, `tt_val`, `tt_dnt`) VALUES (
'sales','".$totalvaluetillnow."','".date('d-m-y',time())."')";

if($conn->query($inserttime)){} else {die("Time updation of total failed");}
                    ...
                    <div class="col-lg-12	">
                        ...
                        <div class="panel panel-default"><!-- /primary heading -->
                            ...
                            <div class="panel-heading">
                                <h3 class="panel-title ">Sales Invoices  - <?php echo $totalvaluetillnow; ?></h3>
                            </div>
                                    ...
                                    <table id="datatable1" class="table table-striped table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th style="width:10%">Proforma <br> Ref</th>
                                                    <th style="width:20%">Client Name</th>
                                                    <th style="width:10%">Total</th>
                                                    <th style="width:20%">Client Payment Terms</th>
                                                    <th style="width:5%">Detailed View</th>
                                                    <th style="width:5%">Print View(No Cost)</th>                                                    
                                                    <th style="width:5%">Generation <br>Date</th>                                                    
                                                    <th style="width:5%">Payment<br>Due Date</th>                                                    
													<?php  if($_SESSION['STWL_LUM_TU_ID'] == '1' or $_SESSION['STWL_LUM_TU_ID'] == '2' ){ ?><th style="width:5%">Action</th><?php }?>
...
$getmax = getdatafromsql($conn, "select * from sw_salesinvoices_max where so_revision_id = ".$productrw['so_id']);

$checkinggen = getdatafromsql($conn,"select * from sw_salesinvoices_gen where sog_rel_so_id = ".$getmax['so_revision_id']." and sog_valid =1 order by sog_id desc limit 1 ");				
...
if(is_array($checkinggen)){
	$discount = $checkinggen['sog_discount'];
	$vat =  ($checkinggen['sog_vat']/100)* ($getpricetotal['ptotal']-$discount);
	$extraprice = $checkinggen['sog_extra_price'];
...
if($getrevision['so_revision'] == 0){	echo'
<button id="getPrintView" data-toggle="modal" data-target="#view-modal" data-id="'.md5($getrevision['so_id']).'" class="btn btn-sm btn-danger">Get Print View</button>
';}
	...
	<td>'.(time() > $productrw['so_due_date'] ? '<i style="color:red">'.date('d-M-Y',$productrw['so_due_date']).'</i>': date('d-M-Y',$productrw['so_due_date'])).'<br>
	'.($_SESSION['STWL_LUM_TU_ID'] == 1 ? '<button id="getDateEdit" data-toggle="modal" data-target="#view-modal" data-id="'.md5($productrw['so_id']).'" class="btn btn-sm btn-warning">Change</button>' :'').'
</td>
	'; if($_SESSION['STWL_LUM_TU_ID'] == '1' or $_SESSION['STWL_LUM_TU_ID'] == '2' ){echo'<td>
<button id="getPoEdit" data-toggle="modal" data-target="#view-modal" data-id="'.md5($productrw['so_id']).'" class="btn btn-sm btn-warning">Revise</button>
	...
	$con++;
                                    ...
                                    <hr>
<div class="row">
<?php  if($_SESSION['STWL_LUM_TU_ID'] == '1' or $_SESSION['STWL_LUM_TU_ID'] == '2'){ ?>
    <div class="col-md-3">
    ...
    <div class="col-md-9" style=" <?php  if($_SESSION['STWL_LUM_TU_ID'] !== '1' and $_SESSION['STWL_LUM_TU_ID'] !== '2'){ ?> margin-left: 100px <?php } ?>">
...
$(document).on('click', '#getDateEdit', function(e){  
     e.preventDefault();
     ...
     $.ajax({
          url: 'page_that_gives_modal_popups_to_pages.php',
          type: 'POST',
          data: 'salesinvoice_edit_date='+uid,
          dataType: 'html'
     })
     .done(function(data){
          console.log(data); 
