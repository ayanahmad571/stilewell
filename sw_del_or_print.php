...
left join sw_deliveryorders q on g.`dog_rel_do_id` = q.do_revision_id
left join sw_currency c on q.do_rel_cur_id = c.cur_id
left join sw_clients ci on q.do_rel_cli_id = ci.cli_id

where md5(g.`dog_id`)= '".$_GET['id']."' and g.dog_valid = 1 and q.do_valid =1 and ci.cli_valid =1  order by do_revision desc limit 1";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
		...
		$getdateofbase = getdatafromsql($conn, "select * from sw_deliveryorders where do_id = ".$row['do_revision_id']." and do_valid =1");
...
<head>
        ...
        <title><?php echo $row['cli_name'].' - '.$row['do_ref'] ?></title>
                            ...
                            <div class="panel-body">
                                ...
                                <div class="row">
                                    ...
                                    <p class="m-t-10"><?php echo $row['dog_extra'] ?></p>
                                        ...
                                        <div class="pull-right">
                                            <p><strong>Date: </strong> <?php echo date('j/n/Y',$getdateofbase['do_dnt']) ?></p>
...
if ($productsresult->num_rows > 0) {
    ...
    while($productrow = $productsresult->fetch_assoc()) {
		$qty2 = ($productrow['di_qty']);

?>
                <tr>
                    <td><?php echo $c; ?></td>
                    <td><?php echo $productrow['pr_name']; ?>
                    <br><?php echo convert_desc($productrow['di_desc']); ?></td>
                    <td><?php echo $qty2 .' '.$productrow['prty_unit']; ?></td>
                </tr>
<?php
  $c++;  }
} else {
